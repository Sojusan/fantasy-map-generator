...
function getVerticalHeaderValuesArray(valuesArray) {
  for (let index = 0; index < precipitationLimitsArray.length + 1; index++) {
    if (index === 0) {
      ...
      valuesArray.push(
        `[${precipitationLimitsArray[index - 1]}, ${precipitationLimitsArray[index]})`
      );
    }
  }
}
...
function createBiomesMatrixChart() {
  ...
  // Add X axis
  const xAxis = d3.scaleBand().domain(horizontalHeaderValuesArray).range([0, width]);
  chartSvg
    .append("g")
  ...
  // Add Y axis
  const yAxis = d3.scaleBand().domain(verticalHeaderValuesArray).range([0, height]);
  chartSvg.append("g").call(d3.axisLeft(yAxis).tickSizeOuter(0));

  // Show the bars
  biomesMatrix
    .slice()
    .reverse()
    .forEach((value, prec_index) => {
...
function createBiomesPolygonsChart() {
  ...
  // Add X axis
  const xAxis = d3.scaleLinear().domain([-50, 50]).range([0, width]);
  chartSvg
    .append("g")
    .attr("transform", `translate(0, ${height})`)
    .call(d3.axisBottom(xAxis).tickSizeOuter(0));
  ...
  // Add Y axis
  const yAxis = d3.scaleLinear().domain([0.0, 1.0]).range([height, 0]);
  chartSvg.append("g").call(d3.axisLeft(yAxis).ticks(6, "f"));
  ...
  chartSvg
    ...
    .join("circle")
    .attr("cx", function (polygon) {
      return xAxis(polygon.temperature);
    })
    .attr("cy", function (polygon) {
      return yAxis(polygon.precipitation);
    })
    .attr("r", 5)
    .style("fill", function (polygon) {
      return polygon.biome.color;
    });
}
