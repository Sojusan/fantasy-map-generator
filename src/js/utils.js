...
function findClosestWithRadius(x, y, radius) {
  ...
  if (
    polygonId !== null &&
    Math.sqrt((x - delaunay.points[polygonId]) ** 2 + (y - delaunay.points[polygonId + 1]) ** 2) <
      radius
  ) {
    return polygonId;
  }
...
function getEdgePoints(polygon, neighbor) {
  ...
  for (let point of polygon) {
    if (
      neighbor.some((neighbor_points) =>
        point.every((value, index) => value === neighbor_points[index])
      )
    ) {
